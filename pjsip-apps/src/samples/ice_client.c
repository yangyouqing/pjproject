...
static void publish_callback(void** unused, struct mqtt_response_publish *published) 
{
    ...
    */
    ...
    if (0 == strcmp (topic_name, JUICE_MQTT_TOPIC_ICE_CLIENT)) {
...
static void* client_refresher(void* client)
{
    ...
    pj_ice_strans_state ice_strans_state;
    ...
    while(1) 
    {
        ...
        if (time(NULL) - last > 1) {
          //  icedemo_show_ice();
            last = time(NULL);
          //  ice_strans_state = pj_ice_strans_get_state(icedemo.icest);
          //  if (PJ_ICE_STRANS_STATE_RUNNING == ice_strans_state) {
                icedemo_send_data(1, "hello, I AM CLIENT\n");
          //  }
        }
    }
    return NULL;
...
int main(int argc, char *argv[])
{
 ...
 /* open the non-blocking TCP socket (connecting to the broker) */
    ...
    mqtt_subscribe(&client, JUICE_MQTT_TOPIC_ICE_CLIENT, 0);    
